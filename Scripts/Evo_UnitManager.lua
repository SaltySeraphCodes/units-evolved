...
dofile "$CONTENT_DATA/Scripts/Evo_survival_units.lua" -- custom units
print("Unite manager evo loaded")
...
local MOD_FOLDER = "$CONTENT_DATA/Scripts"
local chatterDataPath = MOD_FOLDER .."/StreamReaderData/chatterData.json" -- path to chatter data
...
local BotCapsules = {
	...
	sm.uuid.new("1d0f3ad6-a775-47b4-9a74-0100f55039a1") -- Cursed farmbot
}
...
function UnitManager.sv_onCreate( self, overworld, DefaultHostSettingsOverride )
	...
	print("Loading CUstom unit manage for evor")
	...
	if self.saved then
		print( "Loaded UnitManager CUSTOM:" )
		--print( self.saved )
		...
		if savedVersion == 1 then
			...
			print( "Upgraded UnitManager Custom" )
			print( self.saved )
		end
	...
	self.chatterIDs = {}
	self.spawnedChatters = {}
	self:outputData(self.chatterIDs) -- print stuff
	print("Unit manager inited")
end
...
function UnitManager.sv_onPlayerJoined( self, player )
	print( "Custom UnitManager: Player", player.id, "joined" )
...
function UnitManager.sv_onWorldFixedUpdate( self, worldSelf ) -- This doesnt care about raids, 
	--Inform player of incoming raids
	self.newPlayers = {}
...
function isInList(list,target) -- longform search function
...
function findUseridInList(list,target) -- longform search function
	for key, value in pairs(list) do -- For all the values/"items" in the list, do this:
		if value.userid == target then
			return value
		end
	end
	return false
...
function UnitManager.sv_generateUnitChatObject(self,unit,data)
	local obj = {
		userid = data.userid,
...
function UnitManager.sv_addChatterToGame(self,unit,data) -- adds user and outputs
	local unitChatObj = self:sv_generateUnitChatObject(unit,data)
	table.insert( self.chatterIDs, data.userid ) -- add user ids to userids
	table.insert(self.spawnedChatters,unitChatObj)
	self:outputData(self.chatterIDs)
end

function UnitManager.cl_loadUnitData(self,unit) -- Due to race condition, cow will ask for name after being all loaded in [DEPRECIATED BECAUSE NOT NEEDED]
	--print("Got call to load unit data",unit,self.spawnedChatters)
	for k, v in pairs(self.spawnedChatters) do
		if v.unit == unit then
			print("found matching unit")
			sm.event.sendToUnit( unit, "sv_recieveEvent", {event = "setName", data = v.username} )
			sm.event.sendToUnit( unit, "sv_recieveEvent", {event = "setId", data = v.userid} )
			--sm.event.sendToCharacter(unitChar, "sv_recieveEvent", {event = "setName", data = v.username} )
			--unit:sendCharacterEvent( "yolo" ) -- Backups justincase
			print("post unit send, may need character")
		end
    end

end

 --- COMMAND SETTING and event handling
function UnitManager.sv_makeSpeak(self,params) -- finds appropriate unit and makes them speak
	local userid = params.userid
	local unitData = findUseridInList(self.spawnedChatters,userid)
	if unitData == false then
		print("unit not found",userid,unitData,params)
		return
	end
	if sm.exists( unitData.unit ) then  --TODO: if check works
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setChat", data = params.chat} )
	end --
end

function UnitManager.sv_makeFollow(self,params) -- finds appropriate unit and makes them follow target
	...
	if sm.exists( unitData.unit ) then  
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setFollow", data = params.data.target} )
	end
end


function UnitManager.sv_makeAttack(self,params) -- finds appropriate unit to attack
	--print('got attack param',params)
	local userid = params.data.userid
	local unitData = findUseridInList(self.spawnedChatters,userid)
	if unitData == false then
		sm.log.error("users unit not found",userid,unitData,params)
	...
	if sm.exists( unitData.unit ) and unitData.unit ~= params.data.target then  -- dont attack self ends bad
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setAttack", data = params.data.target} )
	end
end


function UnitManager.sv_makeGoto(self,params) -- finds appropriate unit and makes them go to location
	--print("making goto",params)
	local userid = params.data.userid
	local unitData = findUseridInList(self.spawnedChatters,userid)
	if unitData == false then
		sm.log.error("users unit not found",userid,unitData,params)
	...
	if sm.exists( unitData.unit ) then  
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setGoto", data = params.data.target} )
	end
end


function UnitManager.sv_makeFlee(self,params) -- finds appropriate unit and makes them speak
	local userid = params.data.userid
	local unitData = findUseridInList(self.spawnedChatters,userid)
	if unitData == false then
		sm.log.error("users unit not found",userid,unitDataparams)
		return
	end
	if sm.exists( unitData.unit ) then  
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setFlee", data = params.data.target} )
	end
end

function UnitManager.sv_makeStop(self,params) -- makes unit stop
	local userid = params.data.userid
	local unitData = findUseridInList(self.spawnedChatters,userid)
	if unitData == false then
		sm.log.error("users unit not found",userid,unitData,params)
	...
	if sm.exists( unitData.unit ) then  
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "setStop", data = params} )
	end
end

function UnitManager.sv_makeExplode(self,params) -- explodes unit if works?
	...
	--params['world'] = para
	if sm.exists( unitData.unit ) then  
		sm.event.sendToUnit( unitData.unit, "sv_recieveEvent", {event = "explode", data = params} )
		sm.event.sendToWorld(params.world,"sv_e_getExplode",params)
	end
end
...
function UnitManager.sv_removeChatterFromGame(self,userid) -- remove user from known table, possibly implement binary search? or faster method
	for k, v in pairs(self.spawnedChatters) do
		if v.userid == userid then
			table.remove(self.spawnedChatters, k)
		end
    end

	for k, v in pairs(self.chatterIDs) do -- 
		if v == userid then
			table.remove(self.chatterIDs, k) -- should be a flat list
		end
    end
	self:outputData(self.chatterIDs)
end

function UnitManager.sv_UnitSpawned(self,world,unit,params) -- checks and adds unit to list {unit is unit, params is chatter data}
	--print("sv on Spawned new player owned unit",unit,params)
	if params.chatterData ~= nil then
		local userID = params.userid
		params.chatterData['world'] = world -- set world data here
		-- check if exists
		if isInList(self.chatterIDs,userID) == false then -- not already in list
			self:sv_addChatterToGame(unit,params.chatterData) -- takes out chatterData here
		else
			print("chatter already in game",params)
		end
	end
end


function UnitManager.sv_get_spawnedChatters(self) -- returns spawned chatter data
	if self.spawnedChatters then
		print('returning spawned chatters')
		return self.spawnedChatters
	end
end


function UnitManager.outputData(self,data) -- writes data to json file
	--print("outputtting",data,chatterDataPath)
    sm.json.save(data,chatterDataPath)
end


function UnitManager.sv_onRefresh(self,game)
	print("Refreshing unit manager")
	self.chatterIDs = {}
	self.spawnedChatters = {}

	self:outputData(self.chatterIDs) -- clear item
end
