...
function CursedFarmbotUnit.server_onCreate( self )
	self.target = nil
	...
	if self.saved.stats == nil then
		self.saved.stats = { hp = 696969, maxhp = 696969 } -- crazy health
	...
	print( "-- Cursed CursedFarmbotUnit created --" )
end

function CursedFarmbotUnit.server_onRefresh( self )
	print( "-- Cursed CursedFarmbotUnit refreshed --" )
end

function CursedFarmbotUnit.server_onDestroy( self )
	print( "-- Cursed CursedFarmbotUnit terminated --" )
end

function CursedFarmbotUnit.server_onFixedUpdate( self, dt )
	...
	if updateCrushing( self ) then
		print("'CursedFarmbotUnit' was crushed!")
		self:sv_onDeath()
	end
...
function CursedFarmbotUnit.server_onCharacterChangedColor( self, color )
...
function CursedFarmbotUnit.updateAim( self, dt )
...
function CursedFarmbotUnit.server_onUnitUpdate( self, dt )
	...
	if ( done or abortState ) then
		if self.currentState == self.destroyedEventState then
			print("'CursedFarmbotUnit' destroyed!")
			self:sv_onDeath()
...
function CursedFarmbotUnit.sv_e_worldEvent( self, params )
...
function CursedFarmbotUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal, projectileUuid )
...
function CursedFarmbotUnit.server_onMelee( self, hitPos, attacker, damage, power, hitDirection )
	if not sm.exists( self.unit ) or not sm.exists( attacker ) then
		return
	end
	local teamOpponent = false
...
function CursedFarmbotUnit.server_onExplosion( self, center, destructionLevel )
	if not sm.exists( self.unit ) then
		return
	end
...
function CursedFarmbotUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal  )	
	if not sm.exists( self.unit ) then
		return
	end
	...
	if damage > 0 then
		print("'CursedFarmbotUnit' took", damage, "collision damage")
...
function CursedFarmbotUnit.sv_updateCharacterTarget( self )
	if self.unit.character then
		sm.event.sendToCharacter( self.unit.character, "sv_e_updateTarget", { target = self.target } )
	end
end

function CursedFarmbotUnit.sv_takeDamage( self, damage, impact, hitPos )
	if self.saved.stats.hp > 0 then
		...
		print( "'CursedFarmbotUnit' received:", damage, "damage.", self.saved.stats.hp, "/", self.saved.stats.maxhp, "HP" )
...
function CursedFarmbotUnit.sv_e_receiveTarget( self, params )
