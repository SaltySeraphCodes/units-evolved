...
function WocUnit.server_onExplosion( self, center, destructionLevel ) -- because why tf not???
	--print("got exploaded")
	if not sm.exists( self.unit ) then
		return
	end
	...
	if self.unit.character:isTumbling() then -- TODO: can set tumbling here
		ApplyKnockback( self.unit.character, knockbackDirection, 5000 )
	else
		ApplyKnockback( self.unit.character, knockbackDirection, 5000 )
	end
